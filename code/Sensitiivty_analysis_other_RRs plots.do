...
import excel  "${path}data\risk_90pctile\dem_all_reshape_sensitivity.xlsx" , sheet("Sheet1") firstrow clear 
rename indicator race_cat
rename 	(hb nitrate ars tcr dbp pfas lcr) /// 
		(hb_rel_risk nitrate_rel_risk ars_rel_risk tcr_rel_risk dbp_rel_risk pfas_rel_risk lcr_rel_risk)
...
replace border = 2 if type == "Drop Top 10 PWS"
replace border = 3 if type == "Drop Top 1%"
* replace border = 4 if type == "Median"
drop if border == . 
...
replace ind = "Lead ALEs" if indicator == "lcr"
	...
	* Looking at the avg and the 90th together 
	...
	if `"`indicator'"'=="nitrate"  {
		...
		title("`i'")  ///
		legend(pos(6) cols(7) order(1 "Average" 2 "Drop Top 10 PWS" 3 "Drop Top 1%"  ) size(medium)) ///
		scheme(modern)	plotregion(margin(medium))    
		graph export "${output}bar charts\june\bar_`indicator'_sensitivity.png" , as(png) replace
...
********************************************************************************
* Load RRs at the average, 75th, 90th, and 99th percentile 
********************************************************************************
...
gen border = 1 if type == "Average"
*replace border = 2 if type == "75th Percentile"
replace border = 2 if type == "90th Pct."
replace border = 3 if type == "99th Percentile"
replace border = 4 if type == "Median"
drop if border == . 
...
replace ind = "Lead ALEs" if indicator == "lcr"
	...
	* Looking at the avg and the 90th together 
	...
	if `"`indicator'"'=="nitrate"  {
		...
		ytitle("Prevalence Ratio", size(large)) yline(1) ///
		title("`i'")  ///
		legend(pos(6) cols(4) order(1 "Average" 2 "90th Percentile" 3 "99th Percentile" 4 "Median" ) size(medium)) ///
		scheme(modern)	plotregion(margin(medium))    
		graph export "${output}bar charts\june\bar_`indicator'_sensitivity2.png" , as(png) replace
