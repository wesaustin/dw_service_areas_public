...
foreach dem in minorpct frac_white frac_black frac_amerind frac_asian frac_pacisl frac_hisp frac_pov199 {
	...
	preserve
		...
		if `"`dem'"'=="minorpct" | `"`dem'"'=="frac_white" | `"`dem'"'=="frac_black" | `"`dem'"'=="frac_pov199" | `"`dem'"'=="frac_hisp" {
			local k "vtiny"
		}
		if `"`dem'"'=="frac_amerind" | `"`dem'"'=="frac_asian" | `"`dem'"'=="frac_pacisl"  {
			local k "tiny"
		}
	...
	graph export "${path}ej_service_areas\output\scatter\scatter_`dem'.png" , as(png) replace 
