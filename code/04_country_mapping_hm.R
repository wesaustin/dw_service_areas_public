...
cbg_HB_vio <- read_rds("data/combined/HB_vio_hm_area.rds")  %>%
  ...
  mutate(avg_cbg_vio = mean(total_violations),
         cbg_vio_trunc = ifelse(avg_cbg_vio >10, 10, avg_cbg_vio)) %>%
...
HB_vio_plot <- ggplot() + 
  geom_sf(data = HB_vio_all, aes(fill = cbg_vio_trunc, geometry = geometry), 
          color = NA) +
  scale_fill_distiller(name = "", palette = "Reds", 
                       direction = 1, 
...
cbg_lcr_vio <- readRDS("data/combined/lcr_vio_hm_area.rds") #load lcr data
...
cbg_dbp_vio <- read_rds("data/combined/dbp_vio_hm_area_v2.rds") %>%
  ...
  mutate(avg_dbp_cbg = mean(combined_dbp), 
         dbp_cbg_trunc = ifelse(avg_dbp_cbg >= 100, 100, avg_dbp_cbg)) %>%
  distinct(ID, .keep_all = TRUE) 
...
dbp_plot <- ggplot() + 
  ...
  scale_fill_distiller(name = "", palette = "Blues", 
                       direction = 1, limits=c(0,100), breaks = c(0,25,50,75,100), labels = c(0,25,50,75,".100"), 
                       na.value = scales::alpha("#DCDCDC", 0.5)) +
  geom_sf(data = US_st_crop, fill = NA, color = "#969696") +
  coord_sf() +
  theme_void() +
...
cbg_tcr_vio <-read_rds("data/combined/tcr_vio_hm_area.rds") %>%
  ...
  mutate(tcr_det_cbg = mean(detection_share),
         ...
         tcr_det_trunc_20 = ifelse(tcr_det_cbg >= 0.20, 0.20, tcr_det_cbg)) %>%
  distinct(ID, .keep_all = TRUE) %>%
  ungroup
...
summary(cbg_tcr_vio$tcr_det_cbg) #distribution
...
tcr_plot <- ggplot() + 
  ...
  scale_fill_distiller(name = "", palette = "YlOrRd", 
                       direction = 1, limits=c(0,0.10), breaks = c(0, 0.025, 0.05, 0.075, 0.10), labels = c(0,0.025, 0.05,0.075,">0.10"),
                       na.value = scales::alpha("#DCDCDC", 0.5)) +
  geom_sf(data = US_st_crop, fill = NA, color = "#969696") +
  coord_sf() +
  theme_void() +
...
tcr_plot <- ggplot() + 
  geom_sf(data = tcr_vio_all, aes(fill = tcr_det_trunc_20, geometry = geometry), color = NA) +
  scale_fill_distiller(name = "", palette = "YlOrRd", 
                       direction = 1, limits=c(0,0.20), breaks = c(0, 0.05, 0.10, 0.15, 0.20), labels = c(0,0.05, 0.10, 0.15,">0.20"),
                       na.value = scales::alpha("#DCDCDC", 0.5)) +
  geom_sf(data = US_st_crop, fill = NA, color = "#969696") +
  coord_sf() +
  theme_void() +
...
png(file = paste0(plot_path,"tcr_det_share_trunc20_US_hm.png"), 
    width = 1915, height = 1077, units = "px", pointsize = 20,
    bg = "transparent")
