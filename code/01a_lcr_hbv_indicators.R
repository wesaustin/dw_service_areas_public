...
# check observation count for just ALEs 

pws_list <- read_dta("C:/Users/GAUSTIN/OneDrive - Environmental Protection Agency (EPA)/NCEE - Water System Service Boundaries/data/pws_list.dta")

lcr_filtered <- lcr_vio %>%
  filter(CONTAMINANT_CODE == "PB90" & SAMPLE_MEASURE >= 0.015)
# 47103 distinct ALEs for lead
unique_pwsid_count <- lcr_filtered %>%
  summarise(unique_count = n_distinct(PWSID))

lcr_filtered <- lcr_filtered %>%
  rename(pwsid = PWSID)
lcr_filtered <- lcr_filtered %>%
  filter(pwsid %in% pws_list$pwsid)
#20,755
unique_pwsid_count <- lcr_filtered %>%
  summarise(unique_count = n_distinct(pwsid))
#9,980
...
lcr_vio_pwsid <- lcr_vio_2 %>%
  ...
  filter(is.na(avg_pb_level) | avg_pb_level < 7000) %>% # filter out major outliers but keep NAs - why was this necessary?
...
saveRDS(lcr_vio_pwsid, "ej_service_areas/data/indicators/lcr_violations.rds")

write_csv(lcr_vio_pwsid, "ej_service_area/data/indicators/lcr_violations.csv")
...
sdwa_vio <- readRDS("ej_service_areas/data/indicators/health_violations_2015.rds")

sum(sdwa_vio$total_violations)




################################################################################
# Double Check Observation Counts when filtering to systems with service areas 
################################################################################



# check observation count for ALEs 
...
# now for health-based violations 
...
sdwa_vio_2015_filtered <- sdwa_vio_2015 %>%
  filter(pwsid %in% pws_list$pwsid)
  # 10,495

sdwa_vio_2015_filtered <- sdwa_vio_2015_filtered %>%
  mutate(total_dist = sum(dist_vio, na.rm = TRUE))
