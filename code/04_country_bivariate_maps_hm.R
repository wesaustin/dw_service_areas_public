...
cbg_lcr_vio <- read_rds("Data/combined/lcr_vio_hm_area.rds")  %>%
  mutate(ID=str_pad(ID, 12, pad="0"))%>%
  mutate(ID = as.character(ID))  %>%
  group_by(ID) %>%
...
summary(cbg_tcr_vio$minorpct) #distribution

#3rd quartile = 64, so we will use that as a limit for "frequent violators"

action_vector <- c(.01,0.02) #Modify for indicator-specific
...
breaks <- list(bi_x = c("0", ".01", "0.02", "1"),
               bi_y = c("0", "0.11", "0.63", "1"))
...
# Min. 1st Qu.  Median    Mean 3rd Qu.    Max.    NA's 
#    0.00    0.00    0.02    0.42    0.36   57.75   89682 

action_vector <- c(1,10) #arsenic levels of above a common reporting limit and above the MCL of 10. 

## Map 1: Arsenic concentrations and % POC

#Establish biscale comparison: Arsenic and % POC
