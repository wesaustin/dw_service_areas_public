...
foreach var in     tcr_rel_risk ars_rel_risk {
	* name of the file 
	if `"`var'"'=="hb_rel_risk"  {
		local i "Health-based SDWA Violations"
	}
	...
	* name of y axis 
	if `"`var'"'=="hb_rel_risk"  {
		local j "Prevalence Ratio"
	}
	if `"`var'"'=="lcr_rel_risk"  {
		local j "Prevalence Ratio"
	}
	if `"`var'"'=="pfas_rel_risk"  {
		local j "Concentration Ratio"
	}
	if `"`var'"'=="dbp_rel_risk"  {
		local j "Concentration Ratio"
	}
	if `"`var'"'=="tcr_rel_risk"  {
		local j "Detection Share Ratio"
	}
	if `"`var'"'=="ars_rel_risk"  {
		local j "Concentration Ratio"
	}	
	...
	ytitle("Disparity Measure - `j'") yline(1) ///
	legend(pos(6) cols(5) order(1 "County" 2 "Zipcode" 3 "USGS" 4 "EPIC" 5 "EPA ORD"))
...
*import delimited "${path}ej_service_areas\data\rel_risk_by_race_all_v2.csv" , clear 	
import delimited "${path}ej_service_areas\data\rel_risk\rel_risk_pfas_sensitivity.csv", clear 
...
foreach var in total_samples_rel_risk total_samples_2020_rel_risk detection_share_rel_risk detection_share_2020_rel_risk pfoa_and_pfos_rel_risk pfoa_and_pfos_2020_rel_risk concentration_max_rel_risk concentration_max_2020_rel_risk concentration_avg_rel_risk concentration_avg_2020_rel_risk pfas_count_rel_risk pfas_count_2020_rel_risk {
	...
	ytitle("Disparity Measure - Prevalence Ratio") yline(1) ylabel(0 (0.5)3) ///
	legend(pos(6) cols(5) order(1 "County" 2 "Zipcode" 3 "USGS" 4 "EPIC" 5 "EPA ORD"))
...
*import delimited "${path}ej_service_areas\data\rel_risk_by_race_all_v2.csv" , clear 	
import delimited "${path}ej_service_areas\data\rel_risk\rel_risk_dbp_sensitivity.csv", clear 
...
foreach var in combined_dbp_rel_risk combined_dbp_syr3_rel_risk combined_dbp_syr4_rel_risk	{
	...
	graph bar `var' , over(border) over(dem_cat, sort(rorder)) asyvars name( `var' , replace) ///
	ytitle("Disparity Measure - Concentration Ratio") yline(1)  ylabel(0 (0.5)1.5) ///
	legend(pos(6) cols(5) order(1 "County" 2 "Zipcode" 3 "USGS" 4 "EPIC" 5 "EPA ORD"))
...
********************************************************************************
* Histogram of relative risks by demographic group - TCRs
********************************************************************************
...
*import delimited "${path}ej_service_areas\data\rel_risk_by_race_all_v2.csv" , clear 	
import delimited "${path}ej_service_areas\data\rel_risk\rel_risk_tcr_sensitivity.csv", clear 
...
foreach var in detection_share_rel_risk detection_share_syr_rel_risk detection_share_syr4_rel_risk detection_share_2016_rel_risk	{
	if `"`var'"'=="detection_share_rel_risk"  {
		local i "Total Coliform Measure - All Time"
	}
	if `"`var'"'=="detection_share_2016_rel_risk"  {
		local i "Total Coliform Measure - Pre-2016"
	}
	if `"`var'"'=="sample_count_rel_risk"  {
		local i "Total Coliform Measure - Total Samples"
	}
	if `"`var'"'=="detection_share_syr4_rel_risk"  {
		local i "Total Coliform Measure - Six Year Review 4"
	}	

	graph bar `var' , over(border) over(dem_cat, sort(rorder)) asyvars name( `var' , replace) ///
	ytitle("Disparity Measure - Detection Share Ratio") yline(1)  ylabel(0 (0.5)3) ///
	legend(pos(6) cols(5) order(1 "County" 2 "Zipcode" 3 "USGS" 4 "EPIC" 5 "EPA ORD"))
	graph export "${output}bar_`var'_tcr.png" , as(png) replace

}

foreach var in sample_count_rel_risk	{
	if `"`var'"'=="sample_count_rel_risk"  {
		local i "Total Coliform Measure - Total Samples"
	}
	graph bar `var' , over(border) over(dem_cat, sort(rorder)) asyvars name( `var' , replace) ///
	ytitle("Disparity Measure - Detection Share Ratio") yline(1)  ylabel(0 (0.5)2) ///
	legend(pos(6) cols(5) order(1 "County" 2 "Zipcode" 3 "USGS" 4 "EPIC" 5 "EPA ORD"))
	graph export "${output}bar_`var'_tcr.png" , as(png) replace
}
