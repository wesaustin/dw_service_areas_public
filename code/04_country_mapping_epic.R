...
cbg_HB_vio <- read_rds("Data/combined/HB_vio_epic_area.rds")  %>%
...
cbg_lcr_vio <- read_rds("Data/combined/lcr_vio_epic_area.rds") #load lcr data
...
cbg_pfas_vio <- read_rds("Data/combined/pfas_vio_epic_area.rds") %>%
  mutate(ID=str_pad(ID, 12, pad="0")) %>%
...
cbg_dbp_vio <- read_rds("Data/combined/dbp_vio_epic_area_v2.rds") 1%>%
  mutate(ID=str_pad(ID, 12, pad="0")) %>%
  mutate(ID = as.character(ID))
...
cbg_tcr_vio <-read_rds("Data/combined/tcr_vio_epic_area.rds") %>%
  mutate(ID=str_pad(ID, 12, pad="0")) %>%
  mutate(ID = as.character(ID)) %>%
  group_by(ID) %>%
...
cbg_nitrate_vio <- read_rds("Data/combined/nitrate_vio_epic_area.rds")  %>%
  mutate(ID=str_pad(ID, 12, pad="0")) %>%
  group_by(ID) %>%
