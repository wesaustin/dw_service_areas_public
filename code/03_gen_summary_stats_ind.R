...
for (b in boundary) {
  ...
  pfas_vio <- readRDS(paste0('data/combined/pfas_vio_',b ,'.rds')) %>%
    mutate(total.violations = concentration_avg)
...
write.csv(sum_total, "ej_service_areas/data/sum_stats/boundary_sum_stats.csv")

saveRDS(sum_total, "ej_service_areas/data/sum_stats/boundary_sum_stats.rds")


## Make into a latex table

sum_total <- readRDS("ej_service_areas/data/sum_stats/boundary_sum_stats.rds") 
...
indicators<- c(hb="Health-based violations", lcr="Lead ALEs", pfas="PFAs concentration", 
...
kable_table <- kbl(text_table[,2:8], booktabs = T, 
  ...
  pack_rows("PFAS Concentration (2013-2023)", 11, 15) %>%
...
for (b in boundary) {
  ...
  ars_vio <- readRDS(paste0('data/combined/ars_vio_',b ,'.rds')) %>%
    mutate(total.violations = ars_ug)
...
write.csv(sum_total, "ej_service_areas/data/sum_stats/boundary_sum_stats_noZ.csv")

saveRDS(sum_total, "ej_service_areas/data/sum_stats/boundary_sum_stats_noZ.rds")


## Make into a latex table


sum_total <- readRDS("ej_service_areas/data/sum_stats/boundary_sum_stats_noZ.rds") 
#mutate(across(everything(), as.character)) %>%
sum_total$tot_pop <- lapply(signif(sum_total$tot_pop, digits = 2), format, digits = 2, scientific=F, big.mark =  ",")
sum_total[c(5:6)] <- lapply(sum_total[c(5:6)], format, digits = 2, nsmall = 0, big.mark=",", trim=F, scientific = F)
sum_total[c(3:4)] <- lapply(sum_total[c(3:4)], format, digits = 2, nsmall=2, trim = T)
...
################################################################################
## Creating data frames for summary stats : by race 
################################################################################
...
write.csv(sum_total, "ej_service_areas/data/sum_stats/dems_risk_sum_stats.csv")

saveRDS(sum_total, "ej_service_areas/data/sum_stats/dems_risk_sum_stats.rds")
...
sum_total <- readRDS("ej_service_areas/data/sum_stats/dems_risk_sum_stats.rds") 
...
write.csv(df, "ej_service_areas/data/sum_stats/ej_index_summaries.csv")
