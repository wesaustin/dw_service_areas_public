...
# First do some initial tests for the different PFAS formulations

summary(lm(concentration_max ~ total_samples, data = pfas_vio))

summary(lm(detection_share ~ total_samples, data = pfas_vio))

summary(lm(detection_share ~ total_samples + concentration_max, data = pfas_vio))

summary(lm(total_samples ~ pfas_count + concentration_max, data = pfas_vio))

summary(lm(detection_share ~ total_samples + concentration_avg, data = pfas_vio))

pfas_cor <- pfas_vio %>%
  dplyr::select(87:91) %>%
  cor(use='complete.obs')

# Not a ton of correlation between the different PFAS variables, which is interesting

pfas_cov <- pfas_vio %>%
  dplyr::select(87:91) %>%
  cov(use='complete.obs')

## Now to create some more data 

boundary = c("epic", "usgs", "zc", "county", "hm")

vars = c('total_samples', 'total_samples_2020', 'detection_share', 'detection_share_2020', 
         'pfoa_and_pfos',  'pfoa_and_pfos_2020', 'concentration_max', 'concentration_max_2020', 
         'concentration_avg', 'concentration_avg_2020', 'pfas_count', 'pfas_count_2020')
...
write.csv(all_risk, file = "ej_service_areas/data/rel_risk/rel_risk_pfas_sensitivity.csv")
